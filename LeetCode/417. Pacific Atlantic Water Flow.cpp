...
public:
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        for(int i = 0; i < m; i++)
            DFS(heights, pacific, m, n, i, 0), DFS(heights, atlantic, m, n, i, n - 1);
        for(int i = 0; i < n; i++)
            DFS(heights, pacific, m, n, 0, i), DFS(heights, atlantic, m, n, m - 1, i);
        for(int i = 0; i < m; i++)
            for(int j = 0; j < n; j++)
                if(pacific[i][j] && atlantic[i][j]) index.push_back({i, j});
        return index;
    }
};
